...
## 1. Концепция: Repository-Aware Execution OS
**Product Definition:** ForgeOS is a repository-aware execution operating system for software engineering work. It plans, patches, validates, critiques, and produces PR-grade artifacts on real codebases under controlled execution environments.

В отличие от редакторов кода (IDE) или тулзов для чата (AI Assistants), ForgeOS разделяет систему на два независимых слоя:
1. **Cognitive Layer (LLMs):** Модели, которые думают, строят гипотезы, предлагают патчи и спорят с critic.
2. **Execution OS Layer:** Операционная система исполнения, которая поднимает окружение, изолирует ран, ставит зависимости, запускает тесты, собирает артефакты, даёт детерминированные логи и умеет безопасно откатываться.

**Primary System Goal:** Not just code generation quality, but **reliable execution of end-to-end SWE tasks on real repositories**.

**Core Differentiator:** ForgeOS separates cognition from execution: models reason; the Execution OS guarantees controlled environments, reproducibility, validation, and traceable outcomes.

---

## 2. Архитектура системы (High-Level)
Архитектура строится вокруг строгого разделения "мозга" и "машины исполнения".

### 2.1. The Cognitive Layer (The Brains)
...
4. **Multi-Critic Architecture:** Набор экспертов, которые спорят с Planner'ом:
   - *Execution Critic:* Анализирует логи тестов и ошибки среды.
   - *Architecture Critic:* Проверяет соответствие патча паттернам репозитория.
   - *Security Critic:* Ищет уязвимости и утечки секретов.
5. **Anti-Loop Governor:** Гибрид (Heuristic Engine + Small LLM). Контролирует метрики прогресса и меняет стратегии, защищая от циклов.

### 2.2. The Execution OS Layer (System Execution)
Главное ядро системы. Модели только резонируют, Execution OS гарантирует результат:
1. **Run Sandbox:** Изолированные ephemeral workspaces на каждый ран. Git snapshot до/после, возможность детерминированного replay.
2. **Environment Orchestrator:** Автоопределение менеджеров пакетов (`pip / poetry / uv / npm`). Стратегия установки зависимостей, fallback для сломанных инсталляций, system package hints.
3. **Validation Engine:** Запуск тестов разных уровней: smoke, targeted, regression, full suite. Поддержка lint/type/build шагов.
4. **Artifact Ledger:** Хранилище артефактов рана (diff, stderr/stdout, test reports, critic notes, timings, token/cost trace).
5. **Deterministic State Store:** Машина состояний, хранящая точный контекст каждого шага (что знал агент, что предлагал, почему Critic отклонил, что стало причиной финального решения).
6. **Failure Taxonomy Engine:** Классификатор причин сбоев (`ENV_FAILURE`, `DEP_FAILURE`, `TEST_SELECTION_FAILURE`, `PATCH_REGRESSION`, `NONDETERMINISM`). Позволяет движку понимать, *почему* он не смог решить задачу.
