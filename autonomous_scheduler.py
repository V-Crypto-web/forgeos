...
API_BASE           = os.environ.get("FORGEOS_API_BASE", "http://localhost:8082")
SCHEDULER_LOG      = "/tmp/forgeos_scheduler.log"

# ── Load .env so GITHUB_TOKEN etc. are available in-process ──────────────────
def _load_dotenv():
    """Load /opt/ForgeAI/.env (or relative path) into os.environ."""
    _candidates = [
        os.path.join(os.path.dirname(__file__), "..", ".env"),   # relative
        "/opt/ForgeAI/.env",                                      # absolute server path
    ]
            ...
            with open(_path) as _f:
                for _line in _f:
                    ...
                    if _line and not _line.startswith("#") and "=" in _line:
                        ...
                        if _k and _k not in os.environ:   # don't override existing env
                            os.environ[_k] = _v
            break
_load_dotenv()
...
def run_once():
    ...
    candidates = [
        ...
        if item.get("status") not in ("done", "in_progress", "failed") and item["id"] not in failed_epics
    ]

    # ── Opportunity Engine (Low-Backlog Trigger) ──
    if len(candidates) < 3:
        log.info(f"[OPPORTUNITY] Low backlog ({len(candidates)} items) — running Opportunity Engine…")
        try:
            from opportunity_runner import run_opportunity_engine
            # This will detect, score, and materialize real issues
            run_opportunity_engine(dry_run=False)
            
            # Reload backlog with new issues
            backlog = _load_backlog()
            candidates = [
                item for item in backlog
                if item.get("status") not in ("done", "in_progress", "failed") and item["id"] not in failed_epics
            ...
            log.error(f"[OPPORTUNITY] Opportunity Engine failed: {e}")
